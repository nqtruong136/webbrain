...
| **Skill import boundary** | Skills can expose read-only HTTP tools through a `webbrain-tools` manifest. Importing or keeping the skill enabled is the trust decision; declared skill tools run without a per-call permission prompt, use `credentials: "omit"`, and should mark third-party results `resultPolicy: "untrusted"`. |
