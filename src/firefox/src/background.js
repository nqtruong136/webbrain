...
async function handleMessage(msg, sender) {
  ...
  switch (msg.action) {
    ...
    case 'profile_sync_unlock': { const previous = await browser.storage.local.get([PROFILE_SYNC_KEYS.enabled, PROFILE_SYNC_KEYS.metadata]); const reenable = previous[PROFILE_SYNC_KEYS.enabled] !== true && Object.keys(previous[PROFILE_SYNC_KEYS.metadata] || {}).length > 0; await browser.storage.local.set({ [PROFILE_SYNC_KEYS.enabled]: true }); if (reenable) await profileSync.markAllLocalDataChanged(); const state = await profileSync.unlock(String(msg.password || ''), !!msg.create, reenable); await providerManager.load(); return { ok: true, ...state }; }
