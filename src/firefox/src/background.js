...
async function handleMessage(msg, sender) {
  ...
  switch (msg.action) {
    ...
    case 'profile_sync_unlock': { const previous = await browser.storage.local.get([PROFILE_SYNC_KEYS.enabled, PROFILE_SYNC_KEYS.metadata]); const reenable = previous[PROFILE_SYNC_KEYS.enabled] !== true && Object.keys(previous[PROFILE_SYNC_KEYS.metadata] || {}).length > 0; await browser.storage.local.set({ [PROFILE_SYNC_KEYS.enabled]: true }); let state; try { if (reenable) await profileSync.markAllLocalDataChanged(); state = await profileSync.unlock(String(msg.password || ''), !!msg.create, reenable); } catch (error) { await browser.storage.local.set({ [PROFILE_SYNC_KEYS.enabled]: previous[PROFILE_SYNC_KEYS.enabled] === true, [PROFILE_SYNC_KEYS.metadata]: previous[PROFILE_SYNC_KEYS.metadata] || {} }); throw error; } await providerManager.load(); return { ok: true, ...state }; }
