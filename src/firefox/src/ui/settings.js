...
async function requestProfileSyncDataConsent() { const permissions = await browser.permissions.getAll(); if (!Object.hasOwn(permissions, 'data_collection')) return true; return browser.permissions.request({ data_collection: ['personallyIdentifyingInfo', 'authenticationInfo', 'personalCommunications', 'websiteContent', 'technicalAndInteraction'] }); }
async function profileSyncAction(action, data = {}) { try { const result = await sendToBackground(action, data); if (['profile_sync_unlock', 'profile_sync_now', 'profile_sync_reset'].includes(action)) await reloadProfileSyncData(); showProfileSyncResult(true, 'Cloud Sync updated.'); await refreshProfileSyncState(); return result; } catch (error) { showProfileSyncResult(false, error?.message || 'Cloud Sync failed.'); throw error; } }
...
document.getElementById('btn-profile-sync-enable')?.addEventListener('click', async () => { try { const password = checkedSyncPassword(true); if (!await requestProfileSyncDataConsent()) throw new Error('Cloud Sync data permission was not granted.'); await profileSyncAction('profile_sync_unlock', { password, create: true }); } catch (e) { showProfileSyncResult(false, e.message); } });
document.getElementById('btn-profile-sync-unlock')?.addEventListener('click', async () => { try { const password = checkedSyncPassword(); if (!await requestProfileSyncDataConsent()) throw new Error('Cloud Sync data permission was not granted.'); await profileSyncAction('profile_sync_unlock', { password, create: false }); } catch (e) { showProfileSyncResult(false, e.message); } });
