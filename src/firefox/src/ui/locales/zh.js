...
export default {
  ...
  'sp.help_html': '<strong>斜杠命令</strong><br><code>/help</code> — 显示此列表<br><code>/schedule</code> — 创建计划任务<br><code>/list-schedules</code> — 显示计划任务<br><code>/check-progress</code> — 显示当前进度日志<br><code>/show-scratchpad</code> — 显示当前草稿板<br><code>/edit-scratchpad &lt;text&gt;</code> — 将文本追加到当前草稿板<br><code>/clear-scratchpad</code> — 清除当前草稿板<br><code>/remember &lt;text&gt;</code> — 将用户偏好保存到记忆<br><code>/show-memory</code> — 显示已保存的用户记忆<br><code>/forget-memory &lt;id&gt;</code> — 忘记已保存的记忆<br><code>/allow-api</code> — 在本次会话中允许 API 变更<br><code>/dangerously-skip-permissions</code> — 全局禁用权限提示<br><code>/compact</code> — 压缩本次会话的上下文<br><code>/verbose</code> — 切换详细 / 精简工具显示<br><code>/reset</code> — 清除会话<br><code>/screenshot</code> — 截取当前标签页<br><code>/export</code> — 将会话导出为 Markdown<br><code>/export-with-traces</code> — 导出工具链（跟踪）<br><code>/profile</code> — 切换个人资料自动填充<br><code>/vision</code> — 在当前提供商上切换视觉模式<br><code>/ask</code> — 切换到提问模式<br><code>/act</code> — 切换到执行模式<br><code>/dev</code> — 切换到开发模式',
  ...
  'sp.slash.busy_only_oob': 'WebBrain 忙碌时，消息会排队。只有 /help、/check-progress、/show-scratchpad、/show-memory、/list-schedules、/dangerously-skip-permissions、/screenshot、/export、/export-with-traces 和 /verbose 可以作为斜杠命令立即运行。',
  'tool.go_back': '返回',
  'tool.go_forward': '前进',
