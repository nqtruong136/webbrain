...
export default {
  ...
  'sp.help_html': '<strong>スラッシュコマンド</strong><br><code>/help</code> — この一覧を表示<br><code>/schedule</code> — スケジュールタスクを作成<br><code>/list-schedules</code> — スケジュールタスクを表示<br><code>/check-progress</code> — 現在の進捗レジャーを表示<br><code>/show-scratchpad</code> — 現在のスクラッチパッドを表示<br><code>/edit-scratchpad &lt;text&gt;</code> — 現在のスクラッチパッドにテキストを追加<br><code>/clear-scratchpad</code> — 現在のスクラッチパッドをクリア<br><code>/remember &lt;text&gt;</code> — ユーザー設定をメモリに保存<br><code>/show-memory</code> — 保存されたユーザーメモリを表示<br><code>/forget-memory &lt;id&gt;</code> — 保存されたメモリを削除<br><code>/allow-api</code> — この会話で API による変更を許可<br><code>/dangerously-skip-permissions</code> — 権限の確認をグローバルに無効化<br><code>/compact</code> — この会話のコンテキストを圧縮<br><code>/verbose</code> — 詳細／コンパクトなツール表示を切り替え<br><code>/reset</code> — 会話をクリア<br><code>/screenshot</code> — 現在のタブをキャプチャ<br><code>/export</code> — 会話を Markdown でダウンロード<br><code>/export-with-traces</code> — ツールチェーン（トレース）をエクスポート<br><code>/profile</code> — プロフィール自動入力を切り替え<br><code>/vision</code> — アクティブなプロバイダーで画像認識モードを切り替え<br><code>/ask</code> — 質問モードに切り替え<br><code>/act</code> — 操作モードに切り替え<br><code>/dev</code> — Dev モードに切り替え',
